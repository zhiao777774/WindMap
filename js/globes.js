...
var globes = function () {
    ...
    function standardGlobe() {
        return {
             ...
             */
            defineMap: function (mapSvg, foregroundSvg, seaMaskSvg) {
    ...
    function mercator(view) {
        return newGlobe({
            newProjection: function (view) {
                return d3.geo.mercator().scale(1500)
                //.translate([view.width / 2, view.height / 2])
                    .center([0, 24])
                    .rotate([-121, 0, 0]);
            },
            scaleExtent: function () { return [5000, 100000]; },
            locate: function (coord) {
                return [-coord[0], -coord[1], this.projection.rotate()[2]];
            },
            orientation: function (o, view) {
                ...
                var center = projection.center();
                if (µ.isValue(o)) {
                    var parts = o.split(","), λ, φ = +parts[1], scale = +parts[2];
                    if (_.isFinite(parts[0]))
                        λ = +parts[0];
                    var extent = this.scaleExtent();
                    projection.scale(_.isFinite(scale) ? µ.clamp(scale, extent[0], extent[1]) : 1500);
                    projection.translate(this.center(view));
                    projection.center(_.isFinite(φ) ?
                        [0, φ] :
                        this.newProjection(view).center());
                    projection.rotate(_.isFinite(λ) ?
                        [-λ, 0, 0] :
                        this.newProjection(view).rotate());
                    return this;
                }
                return [(-(rotate[0] > 180 ? rotate[0] - 360 : rotate[0] < -180 ? rotate[0] + 360 : rotate[0])).toFixed(2), (center[1]).toFixed(2), Math.round(projection.scale())].join(",");
            },
            manipulator: function (startMouse, startScale) {
                var projection = this.projection;
                var sensitivity = 60 / startScale;  // seems to provide a good drag scaling factor
                ...
                var original = projection.precision();
                var translate = projection.translate();
                var center = [projection.center()[0] / sensitivity, projection.center()[1] / sensitivity];
                return {
                    move: function (mouse, scale) {
                        if (mouse) {
                            /**
                             ...
                             */
                            var xd = mouse[0] - startMouse[0] + rotation[0];
                            var yd = mouse[1] - startMouse[1] + center[1];
                            //projection.translate([translate[0], yd]);
                            projection.rotate([xd * sensitivity, 0]);
                            projection.center([0, yd * sensitivity]);
                            //var xd = mouse[0] - startMouse[0] + rotation[0];
                            //var yd = mouse[1] - startMouse[1] + rotation[1];
                            //projection.rotate([xd * sensitivity, -yd * sensitivity, projection.rotate()[2]]);
                        }
