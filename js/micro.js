...
var µ = function () {
    ...
    var DEFAULT_CONFIG = "current/wind/sfc/mercator";
    var TOPOLOGY = isMobile() ? ["data/earth-topo-mobile.json", "data/twn-topo.json", "data/location.json"] : ["data/earth-topo.json", "data/twn-topo.json", "data/location.json"];
    var requestAnimationFrame = window.requestAnimationFrame ||
        window.mozRequestAnimationFrame ||
        window.webkitRequestAnimationFrame ||
        window.msRequestAnimationFrame ||
        function (callback) {
            var frameCb = function () { callback(+new Date) }
            window.setTimeout(frameCb, 1000 / 60);
        };
    ...
    function colorInterpolator(start, end) {
        var alphaFlag = start.length=== 4 && end.length === 4;
        var r = start[0], g = start[1], b = start[2], a = start[3];
        var Δr = end[0] - r, Δg = end[1] - g, Δb = end[2] - b, Δa = (end[3]) - a;
        return function(i, setAlpha) {
            return [Math.floor(r + i * Δr), Math.floor(g + i * Δg), Math.floor(b + i * Δb), alphaFlag ? Math.floor((a + i * Δa) * 255) : setAlpha];
        };
    }
    //function colorInterpolator(start, end) {
    //    var r = start[0], g = start[1], b = start[2], sa = start[3];
    //    var Δr = end[0] - r, Δg = end[1] - g, Δb = end[2] - b;
    //    return function (i, a) {
    //        return [Math.floor(r + i * Δr), Math.floor(g + i * Δg), Math.floor(b + i * Δb), sa === undefined ? a : sa * 255];
    //    };
    //}
    /**
     ...
     */
    function loadJson(resource, flag = true) {
        var d = when.defer();
        var loadPath = (flag ?
            window.location.pathname.replace('/zh-tw', '') : 'https://cors-anywhere.herokuapp.com/https://www.cwb.gov.tw/cwbwifi/')
            + resource;
     ...
     *     hash   := ( "current" | yyyy / mm / dd / hhhh "Z" ) / param / level [ / option [ / option ... ] ]
    ...
    function parse(hash, projectionNames, overlayTypes) {
        ...
        var tokens = /^(current|(\d{4})\/(\d{1,2})\/(\d{1,2})\/(\d{3,4})Z)\/(\w+)\/(\w+)([\/].+)?/.exec(hash);
        if (tokens) {
            ...
            result = {
                ...
                level: tokens[7],            // non-empty alphanumeric _
                projection: "mercator",
                orientation: "121,24,1500",
                topology: TOPOLOGY,
                overlayType: "default",
                showGridPoints: false
            };
            coalesce(tokens[8], "").split("/").forEach(function (segment) {
    ...
    var Configuration = Backbone.Model.extend({
        ...
        toHash: function () {
            ...
            return [dir, attr.param, attr.level, ol, proj, grid].filter(isTruthy).join("/");
        },
        ...
        sync: function (method, model, options) {
            switch (method) {
                case "read":
                    ...
                    model.set(parse(
                        window.location.hash.substr(1) || model._defaultConfig || DEFAULT_CONFIG,
     ...
     */
    function buildConfiguration(projectionNames, overlayTypes, defaultConfig) {
        var result = new Configuration();
        result._projectionNames = projectionNames;
        result._overlayTypes = overlayTypes;
        result._defaultConfig = defaultConfig;
        return result;
    }
